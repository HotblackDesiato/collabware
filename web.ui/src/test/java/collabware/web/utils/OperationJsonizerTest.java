...
public class OperationJsonizerTest {
	
	private OperationJsonizer<JSONObject, JSONArray> jsonizer = new OperationJsonizer<JSONObject, JSONArray>(new ServerJsonProvider());
	...
	private void assertJsonizedEquals(Operation addNode, String string)	throws JSONException {
		JSONObject nop = jsonizer.jsonize(addNode);
		assertEquals(new JSONObject(string).toString(), nop.toString());
	}
	...
	public void jsonizeSetAttributeOperationWithNumericValues() throws Exception {
		assertJsonizedEquals(setAttr("foobar", "someAttr", (Number) 42, null), "{t:'sa', n:'foobar', a:'someAttr', nv:42}");
	}

	private SetAttributeOperation setAttr(String nodeId, String attributeName, Number newValue, Number oldValue) {
	...
	public void jsonizeSetAttributeOperationWithNumericValuesAndNull() throws Exception {
		assertJsonizedEquals(setAttr("foobar", "someAttr", null, 22), "{t:'sa', n:'foobar', a:'someAttr', ov:22}");
	}
